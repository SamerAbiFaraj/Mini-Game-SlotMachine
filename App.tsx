...
import { initAudio, startSpinSound, playWin, playBigWin, playAmbience, resumeAudioIfNeeded  } from './lib/audio-exports';
...
export default function App() {
  ...
  // simple guard to avoid re‑initing audio constantly
  ...
  useEffect(() => {
    // do NOT initAudio here to satisfy Chrome autoplay
    playAmbience('Calm');
    requestRef.current = requestAnimationFrame(updateGameLoop);
    return () => {
  ...
  const showOverlay =
    (gameState === 'resolvingWin' || gameState === 'animatingBigWin') && lastWin > 0;
    ...
    <div className="relative w-full min-h-[100dvh] bg-black text-white font-body overflow-hidden select-none flex flex-col">
      ...
      >
        <div
          ...
          style={{
            ...
            boxShadow:
              'inset 0 0 200px rgba(0, 240, 255, 0.2), 0 0 300px rgba(0, 240, 255, 0.15)',
          }}
        />
      </div>
      ...
      >
        ...
        <div className="relative mt-2 w-full max-w-2xl flex justify-center flex-shrink min-h-[250px] aspect-[4/3]">
          ...
          />
          <EffectsLayer
            active={gameState === 'resolvingWin' || gameState === 'animatingBigWin'}
            phase={currentPhase}
            wins={lastWinResult?.linesWon || []}
          />

          <WinParticles active={showOverlay} phase={currentPhase} isBigWin={isBigWin} />
        </div>

        {/* Overlay lives at root level, above everything */}
        ...
        {/* Global win confetti overlay */}
        <WinConfetti active={showOverlay} isBigWin={isBigWin} />
